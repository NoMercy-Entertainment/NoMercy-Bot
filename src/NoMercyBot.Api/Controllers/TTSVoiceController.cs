...
public class TtsVoiceController : ControllerBase
{
    ...
    public TtsVoiceController(AppDbContext dbContext, TtsService ttsService, ITtsProviderService providerService)
    {
    ...
    public async Task<IActionResult> GetVoices([FromQuery] string? provider = null, [FromQuery] string? gender = null,
        [FromQuery] string? locale = null)
    {
        // Build query for active voices
        IQueryable<TtsVoice> query = _dbContext.TtsVoices
            .AsNoTracking()
            .Where(v => v.IsActive);

        // Apply provider filter if specified
        if (!string.IsNullOrEmpty(provider)) query = query.Where(v => v.Provider.ToLower() == provider.ToLower());

        // Apply gender filter if specified
        if (!string.IsNullOrEmpty(gender)) query = query.Where(v => v.Gender.ToLower() == gender.ToLower());

        // Apply locale filter if specified
        if (!string.IsNullOrEmpty(locale)) query = query.Where(v => v.Locale.ToLower() == locale.ToLower());
    ...
    public async Task<IActionResult> SetUserVoice([FromBody] SetTtsVoiceDto dto)
    {
        ...
        TtsVoice? voice = await _dbContext.TtsVoices
            .FirstOrDefaultAsync(x => x.SpeakerId == dto.VoiceId && x.IsActive);
    ...
    public async Task<IActionResult> Speak([FromBody] string request)
    {
        ...
        if (string.IsNullOrWhiteSpace(request)) return BadRequest("Request cannot be empty.");
    ...
    public async Task<IActionResult> RefreshProviderVoices()
    {
        try
        {
            ...
            if (allProviders.Count == 0) return NotFound(new { message = "No TTS providers available" });

            foreach (ITtsProvider? provider in allProviders)
            {
                if (provider is not { IsEnabled: true }) continue;

                try
                {
                    List<Services.TTS.Models.TtsVoice> providerVoices = await provider.GetAvailableVoicesAsync();

                    if (providerVoices.Count == 0) continue;

                    // Mark all existing voices for this provider as inactive initially
                    await _dbContext.TtsVoices
                        .Where(v => v.Provider == provider.Name)
                    ...
                    foreach (Services.TTS.Models.TtsVoice providerVoice in providerVoices)
                    {
                        ...
                        await _dbContext.TtsVoices.Upsert(new()
                            {
                                Id = uniqueId,
                ...
                {
                    return StatusCode(500, $"Failed to refresh voices for provider {provider.Name}: {ex.Message}");
                }
            }
