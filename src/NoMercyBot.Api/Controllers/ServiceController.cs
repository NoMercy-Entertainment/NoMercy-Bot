...
public class ServiceController : BaseController
{
    ...
    {
        BotAccount? botAccount = await _dbContext.BotAccounts.FirstOrDefaultAsync();
        if (botAccount == null) return NotFound(new { message = "Bot provider not configured" });
    ...
    {
        if (string.IsNullOrWhiteSpace(token)) return BadRequest("Discord session token cannot be empty");
    ...
    {
        if (string.IsNullOrWhiteSpace(endpoint)) return BadRequest("Azure TTS endpoint cannot be empty");
        await _dbContext.Configurations.Upsert(new()
            {
    ...
    {
        if (string.IsNullOrWhiteSpace(region)) return BadRequest("Azure TTS region cannot be empty");

        await _dbContext.Configurations.Upsert(new()
            {
                Key = "tts_azure_region",
                Value = region
            })
            .On(x => x.Key)
            .WhenMatched((oldConfig, newConfig) => new()
        ...
        return Ok(new
        {
            message = "Azure TTS region updated successfully"
        });
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(key)) return BadRequest("Azure TTS API key cannot be empty");

        await _dbContext.Configurations.Upsert(new()
            {
                Key = "tts_azure_api_key",
                SecureValue = key
            })
            .On(x => x.Key)
            .WhenMatched((oldConfig, newConfig) => new()
    ...
    public async Task<IActionResult> SetTtsCharacterLimit([FromBody] TtsCharacterLimitRequest request)
    {
        if (request.CharacterLimit <= 0) return BadRequest("Character limit must be greater than 0");
        ...
        await _dbContext.Configurations.Upsert(new()
            {
                Key = configKey,
                Value = request.CharacterLimit.ToString()
            })
            .On(x => x.Key)
            .WhenMatched((oldConfig, newConfig) => new()
            {
    ...
    {
        if (request.StartDay < 1 || request.StartDay > 28) return BadRequest("Start day must be between 1 and 28");
        ...
        await _dbContext.Configurations.Upsert(new()
            {
                Key = "tts_billing_cycle_start_day",
                Value = request.StartDay.ToString()
            })
            .On(x => x.Key)
            .WhenMatched((oldConfig, newConfig) => new()
            {
        ...
        return Ok(new
        {
            message = "TTS billing cycle updated successfully"
        });
    }

    [HttpPost("tts-fallback-settings")]
    public async Task<IActionResult> SetTtsFallbackSettings([FromBody] TtsFallbackSettingsRequest request)
    {
        await _dbContext.Configurations.Upsert(new()
            {
        ...
        return Ok(new
        {
            message = "TTS fallback settings updated successfully"
        });
    }
