			...
			try
			{
				...
				if (messageEvent?.Payloads != null)
				{
					foreach (SpotifyMessageEventPayload payload in messageEvent.Payloads)
					{
						...
						{
							if (evt is not { Type: SpotifyEventType.PLAYER_STATE_CHANGED, Event.State: not null })
								continue;
			...
			try
			{
				...
				if (likeEvent?.Payloads != null)
				{
					foreach (string payloadString in likeEvent.Payloads)
					{
						try
						{
							SpotifyLikePayload? payload = JsonConvert.DeserializeObject<SpotifyLikePayload>(payloadString);
							...
							await _widgetEventService.PublishEventAsync("spotify.track.liked", !likedItems.First().Removed);
									
							_logger.LogInformation("Track liked: {Items}", JsonConvert.SerializeObject(likedItems));
						}
						catch (Exception ex)
						{
							_logger.LogError(ex, "Error processing Spotify like event payload");
						}
					}
				}
			}
