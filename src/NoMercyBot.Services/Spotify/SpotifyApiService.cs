...
public class SpotifyApiService
{
    ...
    public async Task<bool> SetVolume(PlayerVolumeRequest request)
    {
        if (request.VolumePercent is < 0 or > 100)
            throw new InvalidOperationException(
                $"Volume must be between 0 and 100. Received: {request.VolumePercent}"
            );
    ...
    public async Task<int> GetVolume()
    {
        ...
        {
            _logger.LogError(
                "Failed to fetch player state: {StatusCode} - {Content}",
                response.StatusCode,
                response.Content
            );
        ...
        if (playerState?.Device?.VolumePercent is null)
            throw new InvalidOperationException(
                "Volume percent is not available in the player state."
            );
    ...
    public async Task<SnapshotResponse> AddToPlaylist(
        string playlistId,
    ...
    )
    {
        ...
        if (request?.Uris == null || !request.Uris.Any())
            throw new ArgumentException(
                "Request must contain at least one URI to add to the playlist.",
                nameof(request)
            );
    ...
    public async Task<Dto.CurrentlyPlaying?> GetCurrentlyPlaying()
    {
        ...
        {
            _logger.LogError(
                "Failed to fetch currently playing track: {StatusCode} - {Content}",
                response.StatusCode,
                response.Content
            );
            throw new InvalidOperationException(
                "Could not retrieve currently playing track from Spotify."
            );
        }
    ...
    public async Task<FullTrack?> GetTrack(string trackId)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to fetch track information for track ID: {TrackId}",
                trackId
            );
            return null;
        }
    ...
    public async Task<FullEpisode?> GetEpisode(string episodeId)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to fetch episode information for episode ID: {TrackId}",
                episodeId
            );
            return null;
        }
    ...
    public async Task<bool> AddToQueue(PlayerAddToQueueRequest request)
    {
        if (request?.Uri == null)
            throw new ArgumentException(
                "Request must contain a valid URI to add to the queue.",
                nameof(request)
            );
    ...
    public async Task InitializeConnectionAsync(string connectionId)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "Discord session token is not set. Cannot retrieve Spotify token."
                );
                return;
            }
            ...
            if (data?.message != "Subscription created")
                throw new InvalidOperationException(
                    $"Error creating subscription: {data?.message}"
                );
    ...
    public async Task<FullPlaylist?> GetPlaylist(string playlistId)
    {
        ...
        _logger.LogInformation(
            "Fetching playlist information for playlist ID: {PlaylistId}",
            playlistId
        );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to fetch playlist information for playlist ID: {PlaylistId}",
                playlistId
            );
            return null;
        }
