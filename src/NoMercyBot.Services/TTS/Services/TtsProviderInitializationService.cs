...
public class TtsProviderInitializationService : IHostedService
{
    ...
    public TtsProviderInitializationService(ITtsProviderService providerService, AppDbContext dbContext)
    {
    ...
    private async Task PopulateProviderVoicesAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (ITtsProvider provider in allProviders)
            try
            {
                ...
                Console.WriteLine($"Updated {providerVoices.Count} voices for provider {provider.Name}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to populate voices for provider {provider.Name}: {ex.Message}");
                continue;
            }

        // Clean up old inactive voices (optional - only remove after a certain period)
        DateTime cutoffDate = DateTime.UtcNow.AddDays(-30);
        int removedCount = await _dbContext.TtsVoices
            .Where(v => !v.IsActive && v.UpdatedAt < cutoffDate && !string.IsNullOrEmpty(v.Provider))
            .ExecuteDeleteAsync(cancellationToken);

        if (removedCount > 0) Console.WriteLine($"Removed {removedCount} old inactive provider voices");
    }
}