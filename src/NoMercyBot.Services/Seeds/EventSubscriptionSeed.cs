...
public static class EventSubscriptionSeed
{
    public static async Task Init(this AppDbContext dbContext)
    {
        ...
        await dbContext.EventSubscriptions.UpsertRange(subscriptions)
            .On(s => new { s.Provider, s.EventType, s.Condition })
    ...
    {
        foreach (KeyValuePair<string, (string Description, string Version, string[][] Conditions)> eventItem in TwitchEventSubService.AvailableEventTypes)
