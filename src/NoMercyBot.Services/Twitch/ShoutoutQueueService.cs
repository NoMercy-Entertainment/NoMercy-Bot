...
public class ShoutoutQueueService : IHostedService
{
    ...
    private readonly ConcurrentDictionary<
        string,
        ConcurrentQueue<ShoutoutRequest>
    > _channelPriorityQueues = new();
    ...
    public async Task CheckIfStreamIsLiveAsync()
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "Cannot check stream status for shoutout queue - TwitchApiService not initialized"
                );
                return;
            }
            ...
            if (streamInfo == null)
            {
                _logger.LogInformation(
                    "Stream is offline on startup - shoutout queue will start when stream goes live"
                );
                return;
            }

            _logger.LogInformation(
                "Stream is already live on startup (started {StartedAt}) - restoring shoutout queue state",
            ...
            List<Channel> recentlyShoutedChannels = await dbContext
            ...
            foreach (Channel c in recentlyShoutedChannels)
            {
                ...
                _logger.LogDebug(
                    "Restored per-user shoutout cooldown for {UserId} (last: {LastShoutout})",
                    c.Id,
                    c.LastShoutout
                );
            }
            ...
            Channel? channel = await dbContext
                .Channels.AsNoTracking()
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to check stream status for shoutout queue: {Message}",
                ex.Message
            );
        }
    }

    public void EnqueueShoutout(
        string channelId,
        string targetUserId,
        string channelName,
        bool isManual,
    ...
    )
    {
        ConcurrentQueue<ShoutoutRequest> targetQueue = isRaid
            ? _channelPriorityQueues.GetOrAdd(
                channelId,
                _ => new ConcurrentQueue<ShoutoutRequest>()
            )
            : _channelQueues.GetOrAdd(channelId, _ => new ConcurrentQueue<ShoutoutRequest>());

        // Check for duplicates across both queues
        ShoutoutRequest[] priorityItems = _channelPriorityQueues.TryGetValue(
            channelId,
            out ConcurrentQueue<ShoutoutRequest>? pq
        )
            ? pq.ToArray()
            : [];
        ShoutoutRequest[] regularItems = _channelQueues.TryGetValue(
            channelId,
            out ConcurrentQueue<ShoutoutRequest>? rq
        )
            ? rq.ToArray()
            : [];
        ShoutoutRequest? existing =
            priorityItems.FirstOrDefault(r => r.TargetUserId == targetUserId)
        ...
        if (existing != null)
        {
            ...
            {
                _logger.LogDebug(
                    "Shoutout for {UserId} already queued in {Channel}, skipping",
                    targetUserId,
                    channelName
                );
            }
    ...
    public async Task OnUserChatMessage(string channelId, string userId, string channelName)
    {
        ...
        try
        {
            ...
            Channel? userChannel = await dbContext
                .Channels.AsNoTracking()
            ...
            _logger.LogInformation(
                "Auto-shoutout triggered for {UserId} in {Channel} (first message this session)",
                userId,
                channelName
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to check channel record for {UserId} in {Channel}: {Message}",
                userId,
                channelName,
                ex.Message
            );
    ...
    {
        _logger.LogInformation(
            "Stream online - resetting shoutout session for {ChannelId}",
            channelId
        );
        ...
        if (
            _channelPriorityQueues.TryGetValue(
                channelId,
                out ConcurrentQueue<ShoutoutRequest>? priorityQueue
            )
        )
    ...
    {
        _logger.LogInformation(
            "Stream offline - clearing shoutout session for {ChannelId}",
            channelId
        );
        ...
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                foreach (string channelId in allChannelIds)
                {
                    ...
                    _channelQueues.TryGetValue(
                        channelId,
                        out ConcurrentQueue<ShoutoutRequest>? regularQueue
                    );
                    ...
                    ConcurrentQueue<ShoutoutRequest> queue = hasPriority
                        ? priorityQueue!
                        : regularQueue!;
                    ...
                    if (
                        !request.IsManual
                        && DateTime.UtcNow - request.EnqueuedAt < AutoShoutoutMessageDelay
                    )
                    {
                    ...
                    )
                    {
                        ...
                        queue.TryDequeue(out _);
                        _logger.LogDebug(
                            "Discarded shoutout for {UserId} in {Channel} - per-user cooldown active",
                    ...
                    if (!success)
                    {
                        ...
                        _logger.LogWarning(
                            "Shoutout for {UserId} failed, re-enqueued",
                            request.TargetUserId
                        );
                    }
                }
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error in shoutout queue processing loop: {Message}",
                    ex.Message
                );
            }
    ...
    private async Task<bool> ExecuteShoutoutAsync(
    ...
    )
    {
        try
        {
            ...
            ChannelInfo channelInfo = await _twitchApiService.GetOrFetchChannelInfo(
            ...
            string chatText = TemplateHelper.ReplaceTemplatePlaceholders(
                template,
                templateCtx,
                isLive,
                gameName,
            ...
            string ttsText = TemplateHelper.ReplaceTemplatePlaceholders(
                template,
                templateCtx,
                isLive,
                gameName,
            ...
            if (!skipApiCall)
            {
                ...
                catch (Exception e)
                {
                    _logger.LogError(
                        e,
                        "Failed to send Twitch shoutout API call for {Username}: {Message}",
                        user.Username,
                        e.Message
                    );
                }
            ...
            try
            {
                await _twitchChatService.SendAnnouncementAsBot(request.ChannelId, chatText);
            ...
            catch (Exception e)
            {
                _logger.LogError(
                    e,
                    "Failed to send shoutout announcement for {Username}: {Message}",
                    user.Username,
                    e.Message
                );
            }
            ...
            await dbContext
                .Channels.Where(c => c.Id == request.TargetUserId)
                .ExecuteUpdateAsync(
                    u =>
                        u.SetProperty(c => c.LastShoutout, DateTime.UtcNow)
            ...
            _logger.LogInformation(
                "Shoutout executed for {Username} in {Channel} (manual: {IsManual})",
                user.Username,
                request.ChannelName,
                request.IsManual
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Failed to execute shoutout for {UserId} in {Channel}: {Message}",
                request.TargetUserId,
                request.ChannelName,
                ex.Message
            );
