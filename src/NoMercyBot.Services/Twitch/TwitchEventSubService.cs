...
public class TwitchEventSubService : IEventSubService
{
    ...
    public async Task<EventSubscription> CreateSubscriptionAsync(string eventType, bool enabled = true)
    {
        ...
        if (existingSub != null)
        {
            ...
            {
                _logger.LogInformation("Notifying about subscription change: {EventType} is now {Status}",
        ...
        EventSubscription subscription = new(ProviderName, eventType, enabled, AvailableEventTypes[eventType].Version)
        {
        ...
        {
            _logger.LogInformation("Notifying about new subscription: {EventType} is now enabled", eventType);
    ...
    public async Task UpdateSubscriptionAsync(string id, bool enabled)
    {
        ...
        if (subscription.Enabled != enabled)
        {
            ...
            {
                _logger.LogInformation("Notifying about subscription update: {EventType} is now {Status}",
                    subscription.EventType, enabled ? "enabled" : "disabled");
    ...
    public async Task UpdateAllSubscriptionsAsync(EventSubscription[] subscriptions)
    {
        ...
        foreach (EventSubscription sub in subscriptions)
        {
            ...
            EventSubscription? existingSub = await _dbContext.EventSubscriptions
                .AsNoTracking() // Use AsNoTracking to avoid change tracking conflicts
                .FirstOrDefaultAsync(s => s.Id == sub.Id);

            if (existingSub != null && existingSub.Enabled != sub.Enabled)
            {
                _logger.LogInformation("Detected subscription change: {EventType} will be {Status}",
                    sub.EventType, sub.Enabled ? "enabled" : "disabled");
        ...
        if (OnEventSubscriptionChanged != null && changedSubscriptions.Count > 0)
        {
            ...
            {
                _logger.LogInformation("Notifying about batch subscription change: {EventType} is now {Status}",
                    kvp.Key, kvp.Value ? "enabled" : "disabled");
    ...
    public async Task DeleteSubscriptionAsync(string id)
    {
        ...
        {
            _logger.LogInformation("Notifying about deleted subscription: {EventType} is now disabled", eventType);
