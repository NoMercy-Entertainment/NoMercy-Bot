...
public class TwitchRewardService
{
    ...
    private void LoadRewardsFromDatabase()
    {
        ...
        foreach (Reward dbReward in dbRewards)
        {
            RegisterReward(new()
            {
                ...
                Permission = Enum.TryParse<RewardPermission>(dbReward.Permission, true, out RewardPermission perm) ? perm : RewardPermission.Everyone,
    ...
    public bool RegisterReward(TwitchReward reward)
    {
        ...
        {
            RewardsById.TryAdd(reward.RewardId, reward);
        }
    ...
    public bool RemoveReward(string identifier)
    {
    ...
    public IEnumerable<TwitchReward> ListRewards() => RewardsById.Values.Concat(RewardsByTitle.Values).Distinct();
    ...
    public async Task ExecuteReward(ChannelPointsCustomRewardRedemptionArgs args)
    {
        ...
        if (reward == null)
        {
            RewardsByTitle.TryGetValue(rewardTitle.ToLowerInvariant(), out reward);
        }
        ...
        if (reward != null)
        {
            ...
            if (!_permissionService.HasMinLevel(userType, reward.Permission.ToString().ToLowerInvariant()))
            {
                ...
                await _twitchApiService.UpdateRedemptionStatus(broadcasterId, twitchRewardId, twitchRedeemId, "CANCELED");
            ...
            RewardContext context = new()
            {
                ...
                FulfillAsync = async () =>
                {
                    await _twitchApiService.UpdateRedemptionStatus(broadcasterId, twitchRewardId, twitchRedeemId, "FULFILLED");
                },
            };
        ...
        {
            _logger.LogDebug("No handler found for reward: {RewardTitle} (ID: {RewardId})", rewardTitle, twitchRewardId);
        }
    }
    ...
    public async Task AddOrUpdateUserRewardAsync(Guid rewardId, string? rewardTitle, string response, string permission = "everyone", bool isEnabled = true, string? description = null)
    {
    ...
    public async Task<bool> RemoveUserRewardAsync(string identifier)
    {
        ...
        Reward? dbReward = await _appDbContext.Rewards.FirstOrDefaultAsync(r => r.Id == rewardId || r.Title == identifier);
